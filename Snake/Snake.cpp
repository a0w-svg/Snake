...
Direction Snake::GetPhysicalDirection()
{
	...
	SnakePart& head = m_snakeBody[0];
	SnakePart& neck = m_snakeBody[1];
...
void Snake::Extend()
{
	...
	SnakePart& tail_head = m_snakeBody[m_snakeBody.size() - 1];
	if (m_snakeBody.size() > 1)
	{
		SnakePart& tail_bone = m_snakeBody[m_snakeBody.size() - 2];
		if (tail_head.position.x == tail_bone.position.x)
		{
			if (tail_head.position.y > tail_bone.position.y)
			{
				m_snakeBody.push_back(SnakePart(
...
void Snake::Reset()
{
	...
	m_snakeBody.push_back(SnakePart(5, 7));
	m_snakeBody.push_back(SnakePart(5, 6));
	m_snakeBody.push_back(SnakePart(5, 5));
...
void Snake::CheckCollision()
{
	...
	SnakePart& head = m_snakeBody.front();
	for (auto itr = m_snakeBody.begin() + 1;
		itr != m_snakeBody.end(); ++itr)
	{
